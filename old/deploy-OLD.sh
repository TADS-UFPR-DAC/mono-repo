...
#----------------------------------------------  RABBITMQ ----------------------------------------------
docker run --network bantads-network -p 15672:15672 -p 5672:5672 --hostname rabbit --name rabbit -d rabbitmq:3-management
#---------------------------------------------- Databases ----------------------------------------------
...
docker run --name postgres-build-read -e POSTGRES_DB=read -e POSTGRES_PASSWORD=password -p 5432:5432 -d postgres #BUILD DATABASE
docker run --name postgres-build-create -e POSTGRES_DB=create -e POSTGRES_PASSWORD=password -p 5432:5432 -d postgres #BUILD DATABASE
...
#-------------------------------------------- Conta Service --------------------------------------------
cd conta-service
mvn spring-boot:build-image #BUILD Conta Service IMAGE
...
#-------------------------------------------- Gerente Service ------------------------------------------
# cd gerente-service
# mvn spring-boot:build-image #BUILD Gerente Service IMAGE
# docker create --network gerente-network --name gerente-service -p 5002:5002 -e DATABASE_HOST=gerente-bd -e RABBIT_HOST=rabbit  gerente-service:0.0.1-SNAPSHOT #DEPLOY CONTAINER
# docker network connect bantads-network gerente-service #ATTACH MAIN NETWORK
# docker start gerente-service
# cd ..
#-------------------------------------------------------------------------------------------------------
#-------------------------------------------- Auth Service --------------------------------------------
cd auth-service
mvn spring-boot:build-image #BUILD Auth Service IMAGE
...
sudo docker container rm -f postgres-build-test
sudo docker container rm -f postgres-build-read
sudo docker container rm -f postgres-build-create
